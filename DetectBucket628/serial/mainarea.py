...
while True:
    ...
    # image process
    maxBox,arrow= proc.process(results,x,y,w,h)
    
    if arrow:
        # get camera coordinate
        prepoint, number,p,depth_intrin = cp.getPoint(maxBox,aligned_depth_frame,x,y)
        pole,head_coordinate,tail_coordinate = cp.transform2world(prepoint, number,p,x,y,depth_intrin)

        if pole:
            centerPoint,slope = cp.completePoint(head_coordinate,tail_coordinate)
        #print(centerPoint)
        # print(slope)
    ...
    ms.sendMsg(mySerial,msgId,centerPoint,slope)
